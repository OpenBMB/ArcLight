/**
 * @file nnml.h
 * @brief Main header file for NNML library, including utility functions, enum, and macros.
 */
#pragma once
...
// universal macros
#define NNML_API_VERSION 1

#ifdef NNML_SHARED
#    if defined(_WIN32) && !defined(__MINGW32__)
#        ifdef NNML_BUILD
#            define NNML_API __declspec(dllexport) extern
#        else
#            define NNML_API __declspec(dllimport) extern
#        endif
#    else
#        define NNML_API __attribute__ ((visibility ("default"))) extern
#    endif
#else
#    define NNML_API extern
#endif

#ifdef __GNUC__
#    define NNML_DEPRECATED(func, hint) func __attribute__((deprecated(hint)))
#elif defined(_MSC_VER)
#    define NNML_DEPRECATED(func, hint) __declspec(deprecated(hint)) func
#else
#    define NNML_DEPRECATED(func, hint) func
#endif

#define NNML_UNUSED(x) (void)(x)
#define NNML_UNUSED_VARS(...) do { (void)sizeof((__VA_ARGS__, 0)); } while(0)

#ifdef __cplusplus
#   define NNML_NORETURN [[noreturn]]
#elif defined(_MSC_VER)
#   define NNML_NORETURN __declspec(noreturn)
#else
#   define NNML_NORETURN _Noreturn
#endif

#ifdef __cplusplus
    // restrict not standard in C++
#    if defined(__GNUC__)
#        define NNML_RESTRICT __restrict__
#    elif defined(__clang__)
#        define NNML_RESTRICT __restrict
#    elif defined(_MSC_VER)
#        define NNML_RESTRICT __restrict
#    else
#        define NNML_RESTRICT
#    endif
#else
#    if defined (_MSC_VER) && (__STDC_VERSION__ < 201112L)
#        define NNML_RESTRICT __restrict
#    else
#        define NNML_RESTRICT restrict
#    endif
#endif


// configuration macros
#define NNML_NUMA_MAX_NODES 8


// debug macros
#ifndef __GNUC__
#    define NNML_ATTRIBUTE_FORMAT(...)
#elif defined(__MINGW32__) && !defined(__clang__)
#    define NNML_ATTRIBUTE_FORMAT(...) __attribute__((format(gnu_printf, __VA_ARGS__)))
#else
#    define NNML_ATTRIBUTE_FORMAT(...) __attribute__((format(printf, __VA_ARGS__)))
#endif

#ifndef NDEBUG
#   define NNML_UNREACHABLE() do { fprintf(stderr, "statement should be unreachable\n"); abort(); } while(0)
#elif defined(__GNUC__)
#   define NNML_UNREACHABLE() __builtin_unreachable()
#elif defined(_MSC_VER)
#   define NNML_UNREACHABLE() __assume(0)
#else
#   define NNML_UNREACHABLE() ((void) 0)
#endif

#define NNML_ABORT(...) nnml_abort(__FILE__, __LINE__, __VA_ARGS__)
#define NNML_ASSERT(x) if (!(x)) NNML_ABORT("NNML_ASSERT(%s) failed", #x)
#define NNML_ASSERT_MSG(cond, msg) do {                                      \
    if (!(cond)) {                                                       \
        fprintf(stderr,                                                  \
                "[NNML ASSERT FAILED]\n"                                 \
                "  Condition: %s\n"                                      \
...
/**
 * debug unitility functions, such as logging and backtrace printing
 */
// printstack
...
static void nnml_print_backtrace_symbols(void) {
    const int max = 100;
    void* buffer[max];
...
inline void nnml_print_backtrace(void) {                   /************** print backtrace **************/
    const char * NNML_NO_BACKTRACE = getenv("NNML_NO_BACKTRACE");
    if (NNML_NO_BACKTRACE) {
        return;
    }
#if defined(__linux__)
    ...
    while ((length = getline(&line, &size, f)) > 0) {
        if (!strncmp(line, "TracerPid:", sizeof("TracerPid:") - 1) &&
            ...
            // Already being debunned, and the breakpoint is the later abort()
            free(line);
            fclose(f);
            return;
        }
...
#else
inline void nnml_print_backtrace(void) {
    // platform not supported
}
#endif
...
// logging
enum nnml_log_level {
    NNML_LOG_LEVEL_NONE  = 0,
    NNML_LOG_LEVEL_DEBUG = 1,
    NNML_LOG_LEVEL_INFO  = 2,
    NNML_LOG_LEVEL_WARN  = 3,
    NNML_LOG_LEVEL_ERROR = 4,
    NNML_LOG_LEVEL_CONT  = 5, // continue previous log
};

NNML_ATTRIBUTE_FORMAT(2, 3)
NNML_API void nnml_log_internal        (enum nnml_log_level level, const char * format, ...);
NNML_API void nnml_log_callback_default(enum nnml_log_level level, const char * text, void * user_data);

#define NNML_LOG(...)       nnml_log_internal(NNML_LOG_LEVEL_NONE , __VA_ARGS__)
#define NNML_LOG_INFO(...)  nnml_log_internal(NNML_LOG_LEVEL_INFO , __VA_ARGS__)
#define NNML_LOG_WARN(...)  nnml_log_internal(NNML_LOG_LEVEL_WARN , __VA_ARGS__)
#define NNML_LOG_ERROR(...) nnml_log_internal(NNML_LOG_LEVEL_ERROR, __VA_ARGS__)
#define NNML_LOG_DEBUG(...) nnml_log_internal(NNML_LOG_LEVEL_DEBUG, __VA_ARGS__)
#define NNML_LOG_CONT(...)  nnml_log_internal(NNML_LOG_LEVEL_CONT , __VA_ARGS__)

#define NNML_DEBUG 0

#if (NNML_DEBUG >= 1)
#define NNML_PRINT_DEBUG(...) NNML_LOG_DEBUG(__VA_ARGS__)
#else
#define NNML_PRINT_DEBUG(...)
#endif

#if (NNML_DEBUG >= 5)
#define NNML_PRINT_DEBUG_5(...) NNML_LOG_DEBUG(__VA_ARGS__)
#else
#define NNML_PRINT_DEBUG_5(...)
#endif

#if (NNML_DEBUG >= 10)
#define NNML_PRINT_DEBUG_10(...) NNML_LOG_DEBUG(__VA_ARGS__)
#else
#define NNML_PRINT_DEBUG_10(...)
#endif

typedef void (*nnml_log_callback)(enum nnml_log_level level, const char * text, void * user_data);

struct nnml_logger_state {
    nnml_log_callback log_callback;
    void * log_callback_user_data;
};
static struct nnml_logger_state g_logger_state = {nnml_log_callback_default, NULL};

static void nnml_log_internal_v(enum nnml_log_level level, const char * format, va_list args) {
    if (format == NULL) {
        return;
    }
    va_list args_copy;
...
inline void nnml_log_internal(enum nnml_log_level level, const char * format, ...) {
    va_list args;
    va_start(args, format);
    nnml_log_internal_v(level, format, args);
    va_end(args);
}

inline void nnml_log_callback_default(enum nnml_log_level level, const char * text, void * user_data) {
    // printf("fc format: %s", text);           // This function will not be used after being launched shortly!
    (void) level;
    (void) user_data;
    fputs(text, stderr);
    fflush(stderr);
...
typedef void (*nnml_abort_callback_t)(const char * error_message);

static nnml_abort_callback_t g_abort_callback = NULL;

// set abort callback (passing null will restore original abort functionality: printing a message to stdout)
NNML_API inline nnml_abort_callback_t nnml_set_abort_callback(nnml_abort_callback_t callback) {
    nnml_abort_callback_t ret_val = g_abort_callback;
    g_abort_callback = callback;
    return ret_val;
}

NNML_NORETURN NNML_ATTRIBUTE_FORMAT(3, 4) inline void nnml_abort(const char * file, int line, const char * fmt, ...) {
    fflush(stdout);
    char message[2048];
...
/**
 * class predeclarations, for other modules to avoid circular dependencies
 */
class nnml_tensor;
class nnml_memory;
typedef std::unique_ptr<nnml_memory> nnml_memory_t;


/**
 * Error: a simple exception class for NNML errors.
 */
...
static inline int nnml_cpu_has_avx(void) {
#if defined(__AVX__)
    return 1;
#else
    return 0;
...
static inline int nnml_cpu_has_avx_vnni(void) {
...
static inline int nnml_cpu_has_avx2(void) {
#if defined(__AVX2__)
    return 1;
#else
    return 0;
...
static inline int nnml_cpu_has_avx512(void) {
...
static inline int nnml_cpu_has_avx512_vbmi(void) {
...
static inline int nnml_cpu_has_avx512_vnni(void) {
...
static inline int nnml_cpu_has_avx512_bf16(void) {
...
static inline int nnml_cpu_has_amx_int8(void) {
...
static inline int nnml_cpu_has_bmi2(void) {
#if defined(__BMI2__)
    return 1;
#else
    return 0;
...
static inline int nnml_cpu_has_fma(void) {
#if defined(__FMA__)
    return 1;
#else
    return 0;
...
static inline int nnml_cpu_has_arm_fma(void) {
...
static inline int nnml_cpu_has_f16c(void) {
#if defined(__F16C__)
    return 1;
#else
    return 0;
...
static inline int nnml_cpu_has_fp16_va(void) {
...
static inline int nnml_cpu_has_sse3(void) {
#if defined(__SSE3__)
    return 1;
#else
    return 0;
...
static inline int nnml_cpu_has_ssse3(void) {
...
static inline int nnml_cpu_has_vsx(void) {
...
static inline int nnml_cpu_has_vxe(void) {
...
static inline int nnml_cpu_has_neon(void) {
...
static inline int nnml_cpu_has_dotprod(void) {
...
static inline int nnml_cpu_has_sve(void) {
...
static inline int nnml_cpu_has_matmul_int8(void) {
...
static inline int nnml_cpu_has_sme(void) {
...
/**
 * low-bit quantization datatype definitions
 */

typedef uint16_t nnml_fp16_t;
typedef struct { uint16_t bits; } nnml_bf16_t;

#define NNML_COMMON_DECL_C
#ifndef NNML_COMMON_DECL

#if defined(NNML_COMMON_DECL_C)
#include <stdint.h>

typedef uint16_t nnml_half;
typedef uint32_t nnml_half2;

#define NNML_COMMON_ANNR_U
#define NNML_COMMON_ANNR_S

#define NNML_COMMON_DECL
#elif defined(NNML_COMMON_DECL_CPP)
#include <cstdint>
...
#define NNML_COMMON_ANNR_U data
// std-c++ do not allow it.
#define NNML_COMMON_ANNR_S data

#define NNML_COMMON_DECL
#endif

#if defined(NNML_COMMON_DECL)
...
#ifdef _MSC_VER
#define NNML_EXTENSION
#else // _MSC_VER
#define NNML_EXTENSION __extension__
#endif // _MSC_VER
...
static_assert(sizeof(block_q4_0) == sizeof(nnml_half) + QK4_0 / 2, "wrong q4_0 block size/padding");

#define QK4_1 32
typedef struct {
    NNML_EXTENSION union {
        struct {
            nnml_half d; // delta
            nnml_half m; // min
        } NNML_COMMON_ANNR_S;
...
static_assert(sizeof(block_q4_1) == 2 * sizeof(nnml_half) + QK4_1 / 2, "wrong q4_1 block size/padding");
...
static_assert(sizeof(block_q5_0) == sizeof(nnml_half) + sizeof(uint32_t) + QK5_0 / 2, "wrong q5_0 block size/padding");
...
static_assert(sizeof(block_q5_1) == 2 * sizeof(nnml_half) + sizeof(uint32_t) + QK5_1 / 2, "wrong q5_1 block size/padding");
...
static_assert(sizeof(block_q8_0) == sizeof(nnml_half) + QK8_0, "wrong q8_0 block size/padding");
...
typedef struct {
    NNML_EXTENSION union {
        struct {
            nnml_half d; // delta
            nnml_half s; // d * sum(qs[i])
        } NNML_COMMON_ANNR_S;
        nnml_half2 ds;
    } NNML_COMMON_ANNR_U;
    int8_t qs[QK8_1]; // quants
} block_q8_1;
static_assert(sizeof(block_q8_1) == 2*sizeof(nnml_half) + QK8_1, "wrong q8_1 block size/padding");
...
static_assert(sizeof(block_tq1_0) == sizeof(nnml_half) + QK_K / 64 + (QK_K - 4 * QK_K / 64) / 5, "wrong tq1_0 block size/padding");
...
static_assert(sizeof(block_tq2_0) == sizeof(nnml_half) + QK_K / 4, "wrong tq2_0 block size/padding");
...
typedef struct {
    ...
    NNML_EXTENSION union {
        struct {
            nnml_half d;    // super-block scale for quantized scales
            nnml_half dmin; // super-block scale for quantized mins
        } NNML_COMMON_ANNR_S;
        nnml_half2 dm;
    } NNML_COMMON_ANNR_U;
} block_q2_K;
static_assert(sizeof(block_q2_K) == 2*sizeof(nnml_half) + QK_K/16 + QK_K/4, "wrong q2_K block size/padding");
...
typedef struct {
    ...
    nnml_half d;           // super-block scale
} block_q3_K;
static_assert(sizeof(block_q3_K) == sizeof(nnml_half) + QK_K / 4 + QK_K / 8 + 12, "wrong q3_K block size/padding");
...
static_assert(sizeof(block_q4_K) == 2*sizeof(nnml_half) + K_SCALE_SIZE + QK_K/2, "wrong q4_K block size/padding");
...
static_assert(sizeof(block_q5_K) == 2*sizeof(nnml_half) + K_SCALE_SIZE + QK_K/2 + QK_K/8, "wrong q5_K block size/padding");
...
static_assert(sizeof(block_q6_K) == sizeof(nnml_half) + QK_K / 16 + 3*QK_K/4, "wrong q6_K block size/padding");
...
// Due to the need to use blocks as per nnml design, it ends up using
// 2.0625 bpw because of the 16-bit scale for each block of 256.
typedef struct {
    nnml_half d;
    uint16_t qs[QK_K/8];
} block_iq2_xxs;
static_assert(sizeof(block_iq2_xxs) == sizeof(nnml_half) + QK_K/8*sizeof(uint16_t), "wrong iq2_xxs block size/padding");
...
static_assert(sizeof(block_iq2_xs) == sizeof(nnml_half) + QK_K/8*sizeof(uint16_t) + QK_K/32, "wrong iq2_xs block size/padding");
...
static_assert(sizeof(block_iq2_s) == sizeof(nnml_half) + QK_K/4 + QK_K/16, "wrong iq2_s block size/padding");
...
static_assert(sizeof(block_iq3_xxs) == sizeof(nnml_half) + 3*(QK_K/8), "wrong iq3_xxs block size/padding");
...
static_assert(sizeof(block_iq3_s) == sizeof(nnml_half) + 13*(QK_K/32) + IQ3S_N_SCALE, "wrong iq3_s block size/padding");
...
static_assert(sizeof(block_iq1_s) == sizeof(nnml_half) + QK_K/8 + QK_K/16, "wrong iq1_s block size/padding");
...
static_assert(sizeof(block_iq4_nl) == sizeof(nnml_half) + QK4_NL/2, "wrong iq4_nl block size/padding");
...
#define NNML_COMMON_IMPL_C
#ifndef NNML_COMMON_IMPL

#if defined(NNML_COMMON_IMPL_C)
#include <stdint.h>

#define NNML_TABLE_BEGIN(type, name, size) static const type name[size] = {
#define NNML_TABLE_END() };

#define NNML_COMMON_IMPL
#elif defined(NNML_COMMON_IMPL_CPP)
#include <cstdint>
...
#if defined(NNML_COMMON_IMPL)

NNML_TABLE_BEGIN(uint8_t, kmask_iq2xs, 8)
    1, 2, 4, 8, 16, 32, 64, 128
NNML_TABLE_END()

NNML_TABLE_BEGIN(uint8_t, ksigns_iq2xs, 128)
...
NNML_TABLE_BEGIN(uint64_t, ksigns64, 128)
...
NNML_TABLE_BEGIN(uint64_t, iq2xxs_grid, 256)
...
NNML_TABLE_BEGIN(uint64_t, iq2xs_grid, 512)
    0x0808080808080808, 0x080808080808082b, 0x0808080808081919, 0x0808080808082b08,
...
NNML_TABLE_BEGIN(uint64_t, iq2s_grid, 1024)
    0x0808080808080808, 0x080808080808082b, 0x0808080808081919, 0x0808080808082b08,
    0x0808080808082b2b, 0x0808080808190819, 0x0808080808191908, 0x080808080819192b,
    0x0808080808192b19, 0x08080808082b0808, 0x08080808082b082b, 0x08080808082b1919,
    0x08080808082b2b08, 0x0808080819080819, 0x0808080819081908, 0x080808081908192b,
...
NNML_TABLE_BEGIN(uint32_t, iq3xxs_grid, 256)
...
NNML_TABLE_BEGIN(uint32_t, iq3s_grid, 512)
...
NNML_TABLE_BEGIN(int8_t, kvalues_iq4nl, 16)
    -127, -104, -83, -65, -49, -35, -22, -10, 1, 13, 25, 38, 53, 69, 89, 113,
NNML_TABLE_END()

// e2m1 values (doubled)
// ref: https://www.opencompute.org/documents/ocp-microscaling-formats-mx-v1-0-spec-final-pdf
NNML_TABLE_BEGIN(int8_t, kvalues_mxfp4, 16)
...
#if defined(NNML_COMMON_IMPL_C)
NNML_TABLE_BEGIN(uint64_t, iq1s_grid, NGRID_IQ1S)
...
NNML_TABLE_END()
#else
NNML_TABLE_BEGIN(uint32_t, iq1s_grid_gpu, NGRID_IQ1S)
